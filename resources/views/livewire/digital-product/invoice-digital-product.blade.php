    ...
    @if($show->status == 'UNPAID')
        ...
        <div class="grid grid-cols-1 gap-4 mt-4 rounded-lg bg-white dark:bg-zinc-600 px-10 py-8">
            <div class="text-center">
                <flux:heading size="xl" class="text-center">Silahkan melakukan pembayaran</flux:heading>
                <flux:button onclick="window.location.href='{{ $show->payment_url }}'" size="sm" variant="primary" class="mt-4">Bayar Sekarang</flux:button>
            </div>
        </div>
    @else
        <div class="flex flex-col md:flex-row gap-6 justify-between md:items-center mb-2">
